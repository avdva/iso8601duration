...
func TestToTimeDuration(t *testing.T) {
	...
	testTime, _ := time.Parse(shortForm, "2015-Sep-23")

	d, err := StringToTimeDuration("P1D", testTime)
	assert.Nil(t, err, nil)
	assert.Equal(t, d, time.Hour*24)

	d, err = StringToTimeDuration("P1M", testTime)
	assert.Nil(t, err, nil)
	assert.Equal(t, d, time.Hour*24*30)
}
